...
This workflow shows how to use the `playlist --sync` command to manage a "to-do" playlist that stays in sync with your track criteria.
...
```bash
djlt list rb "beatgrids:1 && hotcues:<1 && memorycues:<1"
```
...
```bash
djlt playlist --new "Inbox (Simple)"
```
...
```bash
djlt playlist "rb/playlists:name:Inbox (Simple)" \
  --add "rb/tracks:beatgrids:1 && hotcues:<1 && memorycues:<1"
```

!!! tip "All-in-one"
    You can combine the creation and addition steps into a single command:
    ```bash
    djlt playlist --new "Inbox (Simple)" --add "rb/tracks:..."
    ```
...
!!! tip "Dry Run first"
    ...
    ```bash
    djlt playlist "rb/playlists:name:Inbox (Simple)" --sync "..." --dry-run
    ```

```bash
djlt playlist "rb/playlists:name:Inbox (Simple)" \
  --sync "rb/tracks:beatgrids:1 && hotcues:<1 && memorycues:<1"
```
