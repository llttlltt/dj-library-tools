...
If you've moved your music to a new drive, use the `edit` command to repair the paths in bulk.

```bash
# Search a new directory and update paths for all missing tracks
djlt edit rb/tracks --missing --relocate "/Volumes/NewDrive/Music"
```
...
djlt edit m3u8/playlists --repair --file ./my_list.m3u8
```
