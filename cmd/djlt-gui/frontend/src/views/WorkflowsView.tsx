...
export default function WorkflowsView() {
	...
	// Fix 1 — no auto-fetch; opens view mode with step structure only
	...
	// Fix 2 — Preview button: open detail and immediately fetch diff
	...
	// Fix 2 — Play button: open detail and immediately run
	async function openRun(w: Workflow) {
		if (!confirm(`Apply all changes in "${w.name}"? This cannot be undone.`))
			return;
		setError("");
		setDiffs([]);
		setResult(null);
	...
	async function handleSave(wf: Workflow) {
		...
		try {
			...
			setSelected(saved);
			await load();
			// Fix 1 — return to view mode only, no auto-diff fetch
			setMode("view");
		} catch (e) {
			setError(String(e));
		}
	...
	async function handleApply() {
		if (!selected || !confirm("Apply all changes? This cannot be undone."))
			return;
		setBusy(true);
		setError("");
		setMode("applying");
						...
						{wfList.map((w) => (
							...
							>
								<CardHeader className="flex-row items-center justify-between py-3 px-4 gap-0">
									{/* Fix 2 — name area opens view mode */}
	...
	if (mode === "edit" && selected)
		return (
			<WorkflowEditor
				...
				onSave={handleSave}
				onCancel={() => {
					// Fix lint/complexity/noCommaOperator
					if (selected.id) {
						setMode("view");
					} else {
						backToList();
...
function StepViewCard({
	...
	// Fix 3 — show unchanged tracks by default so existing playlist is visible
